...
def read_weather_history_data(spark, path='gs://msds697-solar', file_name_regex=VC_DATA_FILENAME_REGEX):
    """Read weather history CSV from GCS into spark"""
    df = spark.read.format("csv").option("header", "true").option(
        "inferSchema", "true").load(os.path.join(path, file_name_regex))
    return df


def convert_datetime(df, column_name='datetimeEpoch'):
    """Convert unix timestamps to Pacific Time"""
    df = df.withColumn(column_name, from_unixtime(
        column_name).cast('timestamp'))
    return df
...
def read_location_data(spark, path='gs://msds697-solar'):
    ...
    df = spark.read.format("csv").option("header", "true").option(
        "inferSchema", "true").load(os.path.join(path, "OW_coordinate_location.csv"))
    return df
...
def transform_load_to_mongo():
    ...
    weather_df = drop_columns(
        weather_df, columns=['weather_id', 'weather_icon'])
    weather_df = rename_columns(weather_df, {
                                "feelslike": "feels_like", "precip": "precipitation", "severerisk": "severe_risk", "uvindex": "uv_index"})
